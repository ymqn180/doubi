...
check_new_ver(){
	echo -e "${Info} 请输入 Aria2 版本号，格式如：[ 1.34.0 ]，获取地址：[ https://github.com/asdo92/aria2-static-builds/releases ]"
	read -e -p "默认回车自动获取最新版本号:" aria2_new_ver
	if [[ -z ${aria2_new_ver} ]]; then
		aria2_new_ver=$(wget --no-check-certificate -qO- https://api.github.com/repos/asdo92/aria2-static-builds/releases | grep -o '"tag_name": ".*"' |head -n 1| sed 's/"//g;s/v//g' | sed 's/tag_name: //g')
		if [[ -z ${aria2_new_ver} ]]; then
			echo -e "${Error} Aria2 最新版本获取失败，请手动获取最新版本号[ https://github.com/asdo92/aria2-static-builds/releases ]"
...
Download_aria2(){
	...
	wget -N --no-check-certificate "https://github.com/asdo92/aria2-static-builds/releases/download/v${aria2_new_ver}/aria2-${aria2_new_ver}-linux-gnu-${bit}-build1.tar.bz2"
